...
const QuestionsAndAnswers = ({
    ...
    const fetchQa = useCallback(async (page = 1) => {
        ...
        try {
            const response = await productService.getProductQA(
                productStockId,
                productMainStockId,
                page,
                limit
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 mx-auto my-4 bg-white dark:bg-darkSecondary">
            ...
            <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <button
                    ...
                    className={`md:px-6 px-2 py-2 md:h-[3rem] h-[2rem] ${filteredProduct?.length === 0 ? 'grayscale opacity-50' : ''
                        } group relative border-2 border-questionButton text-questionButton hover:border-questionButton/60 hover:bg-questionButton/10 dark:hover:bg-gray-700 rounded-lg flex items-center justify-center gap-2 transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-md`}
                >
                    <div className="flex justify-center items-center gap-1">
                        <MdQuestionMark className="md:text-[1.2rem] text-[1rem]" />
                        <span className="md:text-[0.90rem] text-[0.60rem]">Post Question</span>
                    </div>
                </button>
            </div>
